...
class S3VideoStorage:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(
                "S3 bucket access failed, S3 storage disabled",
            ...
            logger.warning(
                "S3 initialization failed, S3 storage disabled", error=str(e)
            )
            self.s3_client = None
    ...
    async def download_and_store_video(
        ...
        try:
            ...
            logger.info(
                "Video stored successfully in S3",
                s3_url=s3_url,
                size_mb=len(video_data) / (1024 * 1024),
            )
            ...
            logger.error("Failed to upload video to S3", s3_key=s3_key, error=str(e))
            return None
        except Exception as e:
            logger.error(
    ...
    def generate_presigned_url(
        ...
        try:
            ...
            logger.error(
                "Failed to generate presigned URL", s3_key=s3_key, error=str(e)
            )
            return None
